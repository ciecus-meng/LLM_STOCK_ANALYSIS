...
### v2.2.0 (当前版本)
* **修复美股/港股数据接口**：重构了数据获取逻辑，解决了因`akshare`接口对不同市场返回数据格式不一致而导致的程序崩溃问题。
* **增强数据接口健壮性**：为美股数据获取增加了备用数据源（新浪财经），提高了数据可用性。
* **扩展市场扫描功能**：在市场扫描模块中加入了对港股（恒生指数）和美股（标普500、纳斯达克100）主要指数的支持（注：此功能的前后端代码修改方案已提供，需用户手动应用）。
* **代码优化**：修复了多处潜在的`NoneType`和`KeyError`错误，提升了程序的整体稳定性。
